...
public class RecurringScheduleValidator {
    ...
    public void validateCourseSection(CourseSection section) {
        ...
        if (section.getSemester() == null) {
            throw new InvalidDataException("SECTION_NO_SEMESTER", "Lớp học phần chưa thuộc học kỳ nào.");
        }
    ...
    public void validateCourseSectionForOpen(
    ...
    public void validateConflicts(
        ...
        if (recurringScheduleRepository.existsConflictInSection(
                sectionId, dayOfWeek, startPeriod, endPeriod, startWeek, endWeek, excludeId)) {
            throw new InvalidDataException(
                    "SCHEDULE_CONFLICT_SECTION",
                "Lớp học phần này đã có lịch học trùng ngày, tiết và khoảng tuần."
            );
        }

        if (recurringScheduleRepository.existsConflictInClassroom(
                classroomId, semesterId, dayOfWeek, startPeriod, endPeriod, startWeek, endWeek, excludeId)) {
            throw new InvalidDataException(
                    "SCHEDULE_CONFLICT_CLASSROOM",
                "Phòng học đã được sử dụng bởi lớp khác vào cùng ngày, tiết và khoảng tuần trong cùng học kỳ."
            );
        }

        if (recurringScheduleRepository.existsConflictForLecturer(
                lecturerId, semesterId, dayOfWeek, startPeriod, endPeriod, startWeek, endWeek, excludeId)) {
            throw new InvalidDataException(
                    "SCHEDULE_CONFLICT_LECTURER",
                "Giảng viên đã có lịch dạy lớp khác vào cùng ngày, tiết và khoảng tuần trong học kỳ này."
            );
        }
    }
}
